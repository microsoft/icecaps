...
class PersonaTransformerDecoderEstimator(TransformerDecoderEstimator):
    ...
    def init_targets(self):
        ...
        speaker_start_tokens = self.speakers_dense + \
            self.positions[0, :]  # replace _GO token with speaker embedding
        self.inputs_dense = tf.concat(
            [speaker_start_tokens, self.inputs_dense[:, 1:]], 1)

    def get_embdedded_start_tokens(self, effective_batch_size):
